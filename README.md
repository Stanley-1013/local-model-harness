...
位置：`$env:USERPROFILE\.local_model_harness`（無外部依賴，只需 Python 3.9+ 與 Ollama）。

## 常用指令

安裝目錄已加入使用者 PATH，任何新開的 PowerShell / cmd 直接用 `lmh`（舊的完整路徑寫法仍可用）：
...
## 8GB VRAM 建議（本機 RTX 3070 Laptop）
...
- `lmh.cmd` / `lmh.ps1` — `lmh` 短指令包裝（目錄已在使用者 PATH）
